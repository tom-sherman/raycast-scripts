...
Flat directory of one-file scripts. Current inhabitants:

- `bsky-resolve.sh` — resolve a Bluesky handle to a DID
- `warp-connect.sh` / `warp-disconnect.sh` — toggle Cloudflare WARP
- `idasen-desk-move-to-sit.applescript` / `idasen-desk-move-to-stand.applescript` — drive an Idasen standing desk via the Desk Controller app
...
- Shell scripts should pass [ShellCheck](https://www.shellcheck.net/) — the upstream repo lints all contributions with it.
